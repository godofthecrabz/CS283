...
EOF
    ...
    expected_output="assignment_tests.shstudent_tests.shdsh3>dsh3>dsh3>cmdloopreturned0"
...
@test "Redirect out to file and append to the file" {
    rm -f out.txt
    run ./dsh <<EOF
echo "hello, class" > out.txt
echo "how are you today?" >> out.txt
cat out.txt
EOF
    ...
    # Expected output with all whitespace removed for easier matching
    expected_output="hello,classhowareyoutoday?dsh3>dsh3>dsh3>dsh3>cmdloopreturned0"
...
@test "Redirect in from file" {
    rm -f in.txt
    echo "test redirection in" > in.txt
    run ./dsh <<EOF
cat < in.txt
EOF
    ...
    # Expected output with all whitespace removed for easier matching
    expected_output="testredirectionindsh3>dsh3>cmdloopreturned0"
